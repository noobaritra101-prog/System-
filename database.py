    ...
    try:
        ...
        with get_conn() as conn:
            with conn.cursor() as cur:
                ...
                # ⚡ IV SYSTEM: add per-stat IV columns (0-31 each) if missing
                for iv_col in ("iv_hp", "iv_attack", "iv_defense", "iv_sp_atk", "iv_sp_def", "iv_speed"):
                    cur.execute("SELECT column_name FROM information_schema.columns WHERE table_name='pokemons' AND column_name=%s", (iv_col,))
                    if not cur.fetchone():
                        cur.execute(f"ALTER TABLE pokemons ADD COLUMN {iv_col} INTEGER DEFAULT 0")
                conn.commit()
...
def add_caught_pokemon(user_id, name, region, source="Wild"):
    """Adds a caught Pokémon with freshly rolled random IVs (0-31 per stat, Pokétwo-style).
    Returns a dict of the rolled IVs so the caller can show them to the user."""
    ivs = {
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO pokemons (user_id, name, region, iv_hp, iv_attack, iv_defense, iv_sp_atk, iv_sp_def, iv_speed) 
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
            """, (user_id, name, region, ivs["hp"], ivs["attack"], ivs["defense"], ivs["sp_atk"], ivs["sp_def"], ivs["speed"]))
            conn.commit()
    return ivs

def get_pokemon_ivs(user_id, name):
    """Returns the IVs of the most recently caught Pokémon matching this name, plus its overall IV%."""
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT iv_hp, iv_attack, iv_defense, iv_sp_atk, iv_sp_def, iv_speed 
                FROM pokemons WHERE user_id = %s AND name ILIKE %s 
                ORDER BY id DESC LIMIT 1
            """, (user_id, name))
            row = cur.fetchone()
            if not row: return None
            ivs = {"hp": row[0], "attack": row[1], "defense": row[2], "sp_atk": row[3], "sp_def": row[4], "speed": row[5]}
            total_pct = round(sum(ivs.values()) / (31 * 6) * 100, 2)
            return ivs, total_pct
